...
6. Verify that you can login, and also verify with developer tools, that the token is stored in localStorage

<img src="../images/showtoken.png">

7. Now add the code to logout. In the App-component replace the TODO in logout with the code below:
...
8. The only thing missing to complete this simple example, is to make a request, using the token, up against a protected endpoint.
...
9. Now, in the useEffect method in the LoggedIn component, add this call to fetchData added to the facade in the previous step:

```js
facade.fetchData().then(data=> setDataFromServer(data.msg));
```

10. Verify that you can login, and when logged in, that you can fetch data from the protected endpoint on the server.
...
11. Finally, to add the final touch to this small exercise, add error-handling where relevant (wrong credentials when logging in etc.)
